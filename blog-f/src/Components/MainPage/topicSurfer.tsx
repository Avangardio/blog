...
export default function TopicSurfer({currentPage, totalPosts, postsPerPage }: PaginationProps) {
    ...
    const searchParams = useSearchParams().getAll('search');
    ...
    return (
        <div className={'bg-white flex justify-between p-1 w-full box-border align-middle items-center'}>
                <a  href={currentPage === 1 ? "#" : ('/' + `page${currentPage-1}` + `${searchParams.length ? `?search=${searchParams}` : ""}`)}
                    className={'mx-2 rounded-2xl  text-center align-middle text-xl' + ` ${currentPage === 1 ? "text-gray-400 " : " text-cyan-700"}`}
                    onClick={(event) => {
                        event.preventDefault();
                        currentPage !== 1 ? navigation.push('/' + `page${currentPage-1}` + `${searchParams.length ? `?search=${searchParams}` : ""}`) : null;
                    }}
                >
                    {"<"}
                </a>
            <div className={'flex align-middle items-center'}>
                {pageNumbers.map((number, idx) => (
                    <React.Fragment key={number}>
                        ...
                        <a
                            href={'/' + `page${number}` + `${searchParams.length ? `?search=${searchParams}` : ""}`}
                            className={'' +
                            ...
                            onClick={(event) => {console.log(number)
                                event.preventDefault();
                                navigation.push('/' + `page${number}` + `${searchParams.length ? `?search=${searchParams}` : ""}`)
                            }}
                        >
                            {number}
                        </a>
                ...
                <a
                    href={('/' + `page${currentPage+1}` + `${searchParams.length ? `${"?search=" + searchParams}` : ""}`)}
                    className={'mx-2 rounded-2xl  text-center align-middle text-xl' + ` ${currentPage === totalPages ? "text-gray-400 " : " text-cyan-700"}`}
                    onClick={(event) => {
                        event.preventDefault();
                        console.log(searchParams)
                        navigation.push('/' + `page${currentPage+1}` + `${searchParams.length ? `?search=${searchParams}` : ""}`)
                    }}
                >
                    {">"}
                </a>
