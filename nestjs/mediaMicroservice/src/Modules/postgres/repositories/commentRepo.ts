...
import {DatabasePGError, EntityExists } from "@/Errors/postgresErrors/postgresErrors";
...
export default class CommentRepo {
  ...
  findPostComments(post: Post) {
    ...
    }).catch((error) => { throw new DatabasePGError('COMMENTS_ERROR', error.message) })
  }
  async createComment(user: User, post: Post, text: string) {
    ...
    const savedComment = await this.commentRepository.save(comment).catch((error) => {
      // Перехватываем ошибку уникальности
      if (error.code == '23505') {
        throw new EntityExists('DUPLICATE_COMMENT_ERROR', 'A duplicate comment already exists.');
      } else {
  ...
  deleteComment(user: User, post: Post, commentId: number) {
    const {userId} = user;
    const {postId} = post;
    return this.commentRepository.delete({ post: { postId }, user: { userId }, commentId: commentId }).catch((error) => {
