...
export default class LikeRepo {
  constructor(
   ...
   checkUserPostLike(user: User, post: Post) {
    ...
    }).catch((error) => { throw new DatabasePGError('LIKES_ERROR', error.message) })
  }
  async createLike(user: User, post: Post) {
    ...
    const savedLike = await this.likeRepository.save(like).catch((error) => {
        ...
        if (error.code == '23505') {
          throw new EntityExists('DUPLICATE_LIKE_ERROR', 'A duplicate like already exists.');
        } else {
  ...
  deleteLike(user: User, post: Post) {
    const {userId} = user;
    const {postId} = post;
    return this.likeRepository.delete({ post: { postId }, user: { userId } }).catch((error) => {
