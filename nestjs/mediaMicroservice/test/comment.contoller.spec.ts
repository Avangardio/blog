...
describe('[MICROSERVICE] - Медия - комментарии', () => {
    ...
    let commentsLength: number;
        it('[NEST] - Поиск комментариев', async () => {
            ...
            const response = await appController.getComments(getCommentsBody).catch((error) => ErrorHandler(error)) as GetPostCommentsOutput
    ...
    it('[NEST] - Создание комментария', async () => {
        ...
        const response = await appController.createComment(createCommentBody).catch((error) => ErrorHandler(error)) as CreateCommentOutput
    ...
    it('[NEST] - Создание комментария - пользователя нет', async () => {
        ...
        const response = await appController.createComment(createCommentBody).catch((error) => ErrorHandler(error))
    ...
    it('[NEST] - Поиск комментариев - проверка на +2', async () => {
        const getCommentsBody: GetPostCommentsBody = {
            postId: 1
        };
        const response = await appController.getComments(getCommentsBody).catch((error) => ErrorHandler(error)) as GetPostCommentsOutput
    ...
    it('[NEST] - Удаление комментария - комментарий есть', async () => {
        ...
        const response = await appController.deleteComment(deleteCommentBody).catch((error) => ErrorHandler(error)) as GetPostCommentsOutput
        expect(response.code).toBe(201);
    });
